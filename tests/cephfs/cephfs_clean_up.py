...
def wait_for_filesystems_removed(client, fs_names, timeout=300, interval=10):
    ...
    while time.time() < end_time:
        ...
        time.sleep(interval)
    raise CommandFailed(
        f"Filesystems still present after {timeout}s: {remaining}"
    )
...
def run(ceph_cluster, **kw):
    ...
    try:
        ...
        if not fs_util.wait_for_mds_process(
            clients[0], process_name="", ispresent=False, timeout=180, interval=10
        ):
            log.warning("MDS daemons still present after 180s; continuing cleanup")
        out, _ = clients[0].exec_command(sudo=True, cmd="ceph health detail")

...
def rename_file(client, nfs_mount, old_name, new_name):
    ...
    try:
        ...
        raise OperationFailedError(f"Failed to rename file {old_name} to {new_name}: {e}")
...
def permission_to_directory(client, nfs_mount):
    ...
    try:
        ...
        raise OperationFailedError(f"Failed to provide permission to directory {nfs_mount}: {e}")
...
def run(ceph_cluster, **kw):
    ...
    try:
        if operation == "before_upgrade":
            ...
            # Create file in parellel using ThreadPoolExecutor
            log.info("Creating files in parallel using ThreadPoolExecutor after upgrade")
            with ThreadPoolExecutor(max_workers=None) as executor:
                for client in clients:
                    futures = [
                    executor.submit(
                ...
                for future in futures:
                        future.result()
            log.info("All delete files during before upgrade are created successfully after upgrade")

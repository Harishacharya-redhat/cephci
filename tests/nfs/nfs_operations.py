...
def create_nfs_via_file_and_verify(
    ...
    The spec must live on the installer (SFTP + ``orch apply`` both use remote paths).

    After a successful apply and service verification, systemd coredump settings are
    enabled on NFS server nodes via ``Enable_nfs_coredump``. Pass ``nfs_nodes`` when
    known (typical single-host BYOK), or ``cluster_nodes`` (all cluster nodes) to
    resolve hosts from ``ceph orch ps`` for each ``service_id`` in ``nfs_objects``.
    """
    ...
    try:
        ...
        spec_fp.write(
            yaml.dump_all(nfs_objects, sort_keys=False, indent=2).encode("utf-8")
        )
        spec_fp.flush()
    finally:
    ...
    try:
        ...
        if nfs_nodes:
            ...
            nodes_for_coredump = _resolve_nfs_nodes_for_service_ids(
...
@retry((OperationFailedError, MountFailedError), tries=4, delay=5, backoff=2)
